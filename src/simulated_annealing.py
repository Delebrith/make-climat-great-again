...
class SimulatedAnnealing:
    ...
    def _next_iteration(self):
        ...
        adding_probability = len(self._points_set.points_to_add) / \
            len(self._points_set.points_to_add) + len(self._points_set.points_to_remove)

        if random.random() < adding_probability:
            # select point to add
            point = random.sample(self._points_set.points_to_add, 1)
            ...
            point = random.sample(self._points_set.points_to_remove, 1)
            old_value = self._points_set.value
        ...
        self._temperature = self._temperature * math.pow(0.95, self._iterations)
